...
@property (assign, readwrite) NSInteger typeProduct;
...
@synthesize delegate;
@synthesize page=_page, total=_total, results=_results, requestStatus=_requestStatus, pageSize=_pageSize, typeProduct = _typeProduct;

- (id)initWithPageSize:(NSInteger)pageSize delegate:(id<NMPaginatorDelegate>)paginatorDelegate typeProduct:(NSInteger)typeProduct
{
    if(self = [super init])
    {
...
- (void)fetchNextPage
{
    ...
    if(![self reachedLastPage]) {
        ...
        [self fetchResultsWithPage:self.page+1 pageSize:self.pageSize typeProduct:self.typeProduct];
    }
}
